...
class GameboardView : UIView {
    ...
    init(dimension d: Int, tileWidth width: CGFloat, tilePadding padding: CGFloat, cornerRadius radius: CGFloat, fontSize: CGFloat, backgroundColor: UIColor, foregroundColor: UIColor) {
        assert(d > 0)
        dimension = d
        tileWidth = width
        tilePadding = padding
    ...
    func insertTile(at pos: (Int, Int), value: Int) {
        ...
        let tile = TileView(position: CGPoint(x: x, y: y), width: tileWidth, value: value, radius: r, fontSize: fontSize, delegate: provider)
    ...
    func moveOneTile(from: (Int, Int), to: (Int, Int), value: Int) {
        ...
        guard let tile = tiles[fromKey] else {
            ...
            // why was this an error? seems to work without
            return
        }
